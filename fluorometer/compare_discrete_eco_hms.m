...
load ecochl_all.mat %load all in situ fluorometer results (ecofl series sensors)
load CHLatASIT.mat %load discrete sample extracted chl results

% Now, load in the fluorometer deployment info:
filename = 'Fluor_MVCO_deploy_matlab_readable.txt';
delimiter = '\t';
startRow = 2;
...
scatter(dat(:,1), dat(:,7), 5, deploy_match(:,5),'filled') %serial number
cmap=hsv(10);
